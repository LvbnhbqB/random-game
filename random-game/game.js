...
function playPC() {
  ...
  if (randomPC === 0) {
    ...
    setTimeout(() => {
      ...
      winnerText = document.getElementsByClassName(
        "winner-text"
      )[0].textContent = "Who's the winner?";
    }, 1000);
  }
  if (randomPC === 1) {
    pc = "papper";
...
function win(e) {
  ...
  playPC();
  if (player === pc) {
    winnerText = document.getElementsByClassName("winner-text")[0].textContent =
      "GAME DRAW :(";
    ...
    (player === "scissors" && pc === "papper") ||
    (player === "papper" && pc === "rock")
  ) {
    winnerText = document.getElementsByClassName("winner-text")[0].textContent =
      "PLAYER WIN!!!";
